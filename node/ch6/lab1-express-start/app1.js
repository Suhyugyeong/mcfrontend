...
app.get("/", (req, res) => {
  ...
  //현재 스크립트 파일이 위치한 디렉토리에서 "index.html" 파일을 찾아서 클라이언트에게 전송하는 것입니다.
  //이 코드는 클라이언트에게 정적인 HTML 파일을 전송하여 해당 파일이 웹 브라우저에서 렌더링되도록 합니다. 주로 프론트엔드 코드나 정적 파일들을 제공하는 데 사용됩니다.
  //동적인 데이터를 클라이언트에 전송하는 메서드는 res.send()이나 res.json() 등이 있습니다.
  //노트 참고
});
